...
- `ty` (the type-checker) now scans `tests/` in addition to `src/`. The `tests/typing_check.py` `assert_type` contract added in #30 is now enforced automatically; any future PR that breaks an `@overload` signature or a TypedDict shape will fail the pre-commit ty hook. Closes #41.
