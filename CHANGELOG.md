...
- `nwd-dq fetch --format json` now emits a single JSON document (a top-level array of row objects) instead of newline-delimited JSON. Use `--format ndjson` to preserve the previous behavior. The two formats are no longer aliases. Empty results emit `[]` instead of empty output — scripts that branched on "empty output = no rows" should switch to checking exit code or `jq 'length == 0'`. Closes #28.
- Public API now has precise type annotations. `AsyncDataQueryClient.fetch(backend=...)` is typed via `@overload` so editors and type-checkers see the correct return type per backend (`pyarrow.Table` / `polars.DataFrame` / `pandas.DataFrame`) instead of `Any`. `fetch_raw` and `describe` return a `DataQueryPayload = dict[str, LocationEntry]` typed shape instead of `dict[str, Any]`. New TypedDicts (`LocationEntry`, `TimeseriesEntry`) and the `DataQueryPayload` alias are exported from `nwd_dataquery`. No runtime behavior change. Closes #30.
- `ty` (the type-checker) now scans `tests/` in addition to `src/`. The `tests/typing_check.py` `assert_type` contract added in PR #42 is now enforced automatically; any future PR that breaks an `@overload` signature or a TypedDict shape will fail the pre-commit ty hook. Closes #41.
- `AsyncDataQueryClient.fetch_raw` (and `fetch`/`describe`) now validates the *resolved* time window in addition to the explicit-bounds check from #27. A negative `lookback` is rejected directly with `ValueError` (`"lookback must be non-negative"`). A future `start` combined with an omitted `end` (which defaults to `now()`) is caught after default resolution with `ValueError` (`"resolved window is inverted"`). Both fail before any HTTP request. Closes #36.
- Bumped `aia-chaser>=3.4.1` (was `>=3.4.0`). 3.4.1 skips unparsable trust-store certs and suppresses the `CryptographyDeprecationWarning` users were seeing on systems whose root store contains a cert with a non-positive serial number (e.g. macOS). It also removes the future-breakage risk in `_build_ssl_context` once `cryptography` promotes that warning to an error. Closes #5.
